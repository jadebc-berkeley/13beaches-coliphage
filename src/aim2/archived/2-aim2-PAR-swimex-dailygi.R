...
# --------------------------------------
# 2-aim2-PAR-swimex-dailygi.R
# ben arnold (benarnold@berkeley.edu)
#
# description:
# estimate the risk attributable
# to swimming in water using swimmers and
# non-swimmers for days of regular daily activities
# (school, work, vacation) missed due to GI illness
#
...
N.swimex <- function(data) {
	...
	N.all       <- calcNs(data$dailygi)
	N.age0to4   <- calcNs(data$dailygi[data$agestrat=="(0, 4]"])
	N.age5to10  <- calcNs(data$dailygi[data$agestrat=="(4, 10]"])
	N.age11plus <- calcNs(data$dailygi[data$agestrat==">10"])
...
PARswimex.dailygi <- bootAR(fn="ARswimex",fmla=fmla,data=ad,ID=ad$hhid,strata=ad$beach,iter=1000)

# save results down
rm(ad)
save.image(file="~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARswimex-dailygi.RData")
