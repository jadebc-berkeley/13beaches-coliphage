...
# --------------------------------------
# 3-aim2-PAR-swimex-workgi.R
# ben arnold (benarnold@berkeley.edu)
#
# description:
# estimate the risk attributable
# to swimming in water using swimmers and
# non-swimmers for days of paid work
# missed due to GI illness
#
...
N.swimex <- function(data) {
	...
	N.all       <- calcNs(data$workgi)
	N.age0to4   <- calcNs(data$workgi[data$agestrat=="(0, 4]"])
	N.age5to10  <- calcNs(data$workgi[data$agestrat=="(4, 10]"])
	N.age11plus <- calcNs(data$workgi[data$agestrat==">10"])
...
PARswimex.workgi <- bootAR(fn="ARswimex",fmla=fmla,data=ad,ID=ad$hhid,strata=ad$beach,iter=1000)

# save results down
rm(ad)
save.image(file="~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARswimex-workgi.RData")
