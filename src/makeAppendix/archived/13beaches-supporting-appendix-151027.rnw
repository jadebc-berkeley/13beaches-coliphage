...
% R preamble
%--------------------------------------------------------
<<'R preamble', eval=TRUE, echo=FALSE>>=
# --------------------------------------
# R preamble
# --------------------------------------
...
setwd("~/dropbox/13beaches/src/makeAppendix/")
#Sys.setenv(TEXINPUTS=getwd(),
#           BIBINPUTS="~/dropbox/Library/",
#           BSTINPUTS="~/library/texmf/bibtex/"
#          )
...
\hline
<<'wq table sample counts',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim1-results/rawoutput/aim1-wq-summary-tables.Rdata")
sdata <- cbind(wqtab.1600[,1:2],wqtab.qpcr[,1:2])
stot <- apply(sdata,2,sum)
rowtable(sdata,c(0,0,0,0,0))
@
\hline
<<'wq table sample counts total',echo=FALSE,results='asis'>>=
stotal <- data.frame(t(stot))
rownames(stotal) <- "Total"
rowtable(stotal,c(0,0,0,0,0))
@
\hline
\end{tabular}
...
\hline
<<'wq table entero1600',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim1-results/rawoutput/aim1-wq-summary-tables.Rdata")
rowtable(wqtab.1600,c(0,0,0,1,0,0))
@
\hline
<<'wq total entero1600',echo=FALSE,results='asis'>>=
tot1600 <- data.frame(t(total.1600))
rownames(tot1600) <- "Total"
rowtable(tot1600,c(0,0,0,1,0,0))
@
\hline
\end{tabular}
\end{center}
...
-Locations & Samples & Non-Detects &     &     & Mean \\
\hline
<<'wq table enteroQPCR',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim1-results/rawoutput/aim1-wq-summary-tables.Rdata")
rowtable(wqtab.qpcr,c(0,0,0,1,0,0))
@
\hline
<<'wq total enteroQPCR',echo=FALSE,results='asis'>>=
totqpcr <- data.frame(t(total.qpcr))
rownames(totqpcr) <- "Total"
rowtable(totqpcr,c(0,0,0,1,0,0))
@
\hline
\end{tabular}
\end{center}
...
\begin{tabular}{l rrr rrr rrr rrr}
 ...
 % & N & \% & Median (IQR) &  N & \% & Median (IQR) & N & \% & Median (IQR) & N & \% & Median (IQR) \\
  ...
  &  &  & (IQR) &  &  &  (IQR) &  &  &  (IQR) &  &  &  (IQR) \\
\hline
<<'descriptive table',echo=FALSE,results='asis'>>=
...
%--------------------------------------------------------
% Section 3 - Beach Specific Results
%--------------------------------------------------------
\clearpage
\section{Beach-Specific Results}

<<'beach homogeneity tests', eval=TRUE, echo=FALSE, results='hide' >>=
# load the homogeneity test results
load("~/dropbox/13beaches/aim1-results/rawoutput/aim1-homogeneity-tests.RData")
@
...
\Sexpr{sprintf("%1.2f",bI[1])}, 
Pr(Q,df=12)=\Sexpr{sprintf("%1.2f",bI[2])},
$I^2=$ \Sexpr{sprintf("%1.0f",bI[3])}\%),
 head immersion (Cohen's Q=
 \Sexpr{sprintf("%1.2f",hI[1])}, 
Pr(Q,df=12)=\Sexpr{sprintf("%1.2f",hI[2])},
$I^2=$ \Sexpr{sprintf("%1.0f",hI[3])}\%),
 and swallowed water (Cohen's Q=
 \Sexpr{sprintf("%1.2f",sI[1])}, 
Pr(Q,df=12)=\Sexpr{sprintf("%1.2f",sI[2])},
$I^2=$ \Sexpr{sprintf("%1.0f",sI[3])}\%).
...
\Sexpr{sprintf("%1.2f",entero1600I[1])}, 
Pr(Q,df=12)=\Sexpr{sprintf("%1.2f",entero1600I[2])},
$I^2=$ \Sexpr{sprintf("%1.0f",entero1600I[3])}\%),
 EPA qPCR 1611 (Cohen's Q=
 \Sexpr{sprintf("%1.2f",enteroQPCRI[1])}, 
Pr(Q,df=12)=\Sexpr{sprintf("%1.2f",enteroQPCRI[2])},
$I^2=$ \Sexpr{sprintf("%1.0f",enteroQPCRI[3])}\%).
...
%--------------------------------------------------------
% Section 4 -  Enterococcus Associations with Diarrhea
%--------------------------------------------------------
\clearpage
...
%--------------------------------------------------------
% Section 5 - Swim and enterococcus exposure associations with gastrointestinal illness
%--------------------------------------------------------
\clearpage
...
\textbf{Enterococcous} \\
\textbf{Quartiles} \\
<<'negative control entero quartiles culture',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim1-results/rawoutput/aim1-sens-entero1600-Quartile-regs-noswim.Rdata")
ciformat <- function(x) {
  paste(sprintf("%1.0f",x[1]*1000)," (",sprintf("%1.0f",x[2]*1000),", ",sprintf("%1.0f",x[3]*1000),")",sep="")
}
CIform <- apply(ci.all,1,ciformat)
cirformat <- function(x) {
  paste(sprintf("%1.2f",x[1])," (",sprintf("%1.2f",x[2]),", ",sprintf("%1.2f",x[3]),")",sep="")
}
CIRform <- apply(cir.all,1,cirformat)
CIRform <- c("ref",CIRform)

enteroQtab <- cbind(format(N.all[,c(2,1)],big.mark=","),CIform,CIRform)
rownames(enteroQtab) <- paste("Q",1:4,sep="")
rowtable(enteroQtab,digits=0)
@
& \\
\textbf{Enterococcus} \\
<<'negative control entero 35cfu culture',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim1-results/rawoutput/aim1-sens-entero1600-35cfu-regs-noswim.Rdata")
CIform <- apply(ci.all,1,ciformat)
CIRform <- cirformat(cir.all)
CIRform <- c("ref",CIRform)
entero35tab <- cbind(format(N.all[,c(2,1)],big.mark=","),CIform,CIRform)
rownames(entero35tab) <- c("$\\leq35$ CFU/100ml",">35 CFU/100ml")
rowtable(entero35tab,digits=0)
@
& \\
\textbf{Enterococcous qPCR} \\
\textbf{Quartiles} \\
<<'negative control entero quartiles qpcr',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim1-results/rawoutput/aim1-sens-enteroQPCR-Quartile-regs-noswim-negcontrol.Rdata")
CIform <- apply(ci.all,1,ciformat)
CIRform <- apply(cir.all,1,cirformat)
CIRform <- c("ref",CIRform)
enteroqpcrQtab <- cbind(format(N.all[,c(2,1)],big.mark=","),CIform,CIRform)
rownames(enteroqpcrQtab) <- paste("Q",1:4,sep="")
rowtable(enteroqpcrQtab,digits=0)
@
& \\
\textbf{Enterococcus qPCR} \\
<<'negative control entero 470cce qpcr',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim1-results/rawoutput/aim1-sens-enteroQPCR-470cce-regs-noswim-negcontrol.Rdata")
CIform <- apply(ci.all,1,ciformat)
CIRform <- cirformat(cir.all)
CIRform <- c("ref",CIRform)
entero470tab <- cbind(format(N.all[,c(2,1)],big.mark=","),CIform,CIRform)
rownames(entero470tab) <- c("$\\leq470$ CCE/100ml",">470 CCE/100ml")
rowtable(entero470tab,digits=0)
@
& \\
\hline
\end{tabular}
...
%   \item Population Attributable Risk is the number of events per 1000 beachgoers that would be prevented if swimming exposure were removed from the population. The proportion exposed to body immersion swimming was: all ages (\Sexpr{pctexp[1]}), ages 0-4 (\Sexpr{pctexp[2]}), ages 5-10 (\Sexpr{pctexp[3]}), ages >10 (\Sexpr{pctexp[4]}). Table \ref{tab:ptable} includes additional details.
...
\clearpage
<<'Percent Exp age Entero 1600 noswim',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600noswim-diar.Rdata")
nswim <- sum(N.all[,2])
pctexp.all <- paste(sprintf("%1.0f",(N.all[3,2]/sum(N.all[,2]))*100),"\\%",sep="")
pctexp.0to4 <- paste(sprintf("%1.0f",(N.age0to4[3,2]/sum(N.age0to4[,2]))*100),"\\%",sep="")
pctexp.5to10 <- paste(sprintf("%1.0f",(N.age5to10[3,2]/sum(N.age5to10[,2]))*100),"\\%",sep="")
pctexp.11plus <- paste(sprintf("%1.0f",(N.age11plus[3,2]/sum(N.age11plus[,2]))*100),"\\%",sep="")
pctexp <- c(pctexp.all,pctexp.0to4,pctexp.5to10,pctexp.11plus)
@
\begin{table}[h!tb]
\begin{footnotesize}
\begin{center}
...
& \\
\textbf{Diarrhea, episodes} \\
<<'AR watexp noswim diarrhea overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600noswim-diar.Rdata")
printPAR(AR35noswim.diar,colSums(N.all),label="All Ages")
@
Age Stratified \\
<<'AR watexp noswim diarrhea overall ages',echo=FALSE,results='asis'>>=
printPAR(AR35noswim.diar.0to4,colSums(N.age0to4),label="~~Ages 0 to 4")
printPAR(AR35noswim.diar.5to10,colSums(N.age5to10),label="~~Ages 5 to 10")
printPAR(AR35noswim.diar.11plus,colSums(N.age11plus),label="~~Ages >10")
@
& \\
\textbf{Gastrointestinal} \\
\textbf{illness$^4$, episodes} \\
<<'AR watexp noswim gi overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600noswim-gi.Rdata")
printPAR(AR35noswim.gi,colSums(N.all),label="All Ages")
@
Age Stratified \\
<<'AR watexp noswim gi overall ages',echo=FALSE,results='asis'>>=
printPAR(AR35noswim.gi.0to4,colSums(N.age0to4),label="~~Ages 0 to 4")
printPAR(AR35noswim.gi.5to10,colSums(N.age5to10),label="~~Ages 5 to 10")
printPAR(AR35noswim.gi.11plus,colSums(N.age11plus),label="~~Ages >10")
@
& \\
\textbf{Missed Daily} \\
\textbf{Activities$^5$, days} \\
<<'AR watexp noswim dailygi overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600noswim-dailygi.Rdata")
printPAR(AR35noswim.dailygi,colSums(N.all),label="All Ages")
@
Age Stratified \\
<<'AR watexp noswim dailygi overall ages',echo=FALSE,results='asis'>>=
# TEMP CODE
# zero-out a few unrealistic estimates that weren't filtered in ARboot
#AR35noswim.dailygi.0to4$stats[(AR35cfu.dailygi.0to4$stats[,3]>10), ] <- NA
#   AR35noswim.dailygi.0to4$bootest  <- apply(AR35cfu.dailygi.0to4$stats,2,function(x) mean(x,na.rm=T))
#   AR35noswim.dailygi.0to4$boot95lb <- apply(AR35cfu.dailygi.0to4$stats,2,function(x) quantile(x,prob=c(0.025),na.rm=T))
#   AR35noswim.dailygi.0to4$boot95ub <- apply(AR35cfu.dailygi.0to4$stats,2,function(x) quantile(x,prob=c(0.975),na.rm=T))
printPAR(AR35noswim.dailygi.0to4,colSums(N.age0to4),label="~~Ages 0 to 4")
printPAR(AR35noswim.dailygi.5to10,colSums(N.age5to10),label="~~Ages 5 to 10")
printPAR(AR35noswim.dailygi.11plus,colSums(N.age11plus),label="~~Ages >10")
@
& \\
\textbf{Missed Paid} \\
\textbf{Work$^6$, days} \\
<<'AR watexp noswim workgi overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600noswim-workgi.Rdata")
printPAR(AR35noswim.workgi,colSums(N.all),label="All Ages\\textsuperscript{8}")
@
& \\
\textbf{Medical Visits$^7$,} \\
\textbf{events} \\
<<'AR watexp noswim medgi overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600noswim-medgi.Rdata")
printPAR(AR35noswim.medgi,colSums(N.all),label="All Ages\\textsuperscript{8}")
@
& \\
\hline
\end{tabular}
...
\begin{enumerate}
  ...
  \item Population Attributable Risk is the number of events per 1000 swimmers that would be prevented if the exposure of swimming in water with \textit{Enterococcus} $\geq$35 CFU/100ml were removed from the population. The proportion of beachgoers who swam in water with \textit{Enterococcus} >35 CFU/100ml was: all ages (\Sexpr{pctexp[1]}), ages 0-4 (\Sexpr{pctexp[2]}), ages 5-10 (\Sexpr{pctexp[3]}), ages >10 (\Sexpr{pctexp[4]}). 
...
\clearpage
<<'Percent Exp age Entero 1600',echo=FALSE,results='asis'>>=
...
pctexp.0to4 <- paste(sprintf("%1.0f",(N.age0to4[2,2]/sum(N.age0to4[,2]))*100),"\\%",sep="")
pctexp.5to10 <- paste(sprintf("%1.0f",(N.age5to10[2,2]/sum(N.age5to10[,2]))*100),"\\%",sep="")
pctexp.11plus <- paste(sprintf("%1.0f",(N.age11plus[2,2]/sum(N.age11plus[,2]))*100),"\\%",sep="")
pctexp <- c(pctexp.all,pctexp.0to4,pctexp.5to10,pctexp.11plus)
@
\begin{table}[h!tb]
\begin{footnotesize}
...
& \\
\textbf{Diarrhea, episodes} \\
<<'AR watexp diarrhea overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600-diar.Rdata")
printPAR(AR35cfu.diar,colSums(N.all),label="All Ages")
@
Age Stratified \\
<<'AR watexp diarrhea overall ages',echo=FALSE,results='asis'>>=
printPAR(AR35cfu.diar.0to4,colSums(N.age0to4),label="~~Ages 0 to 4")
printPAR(AR35cfu.diar.5to10,colSums(N.age5to10),label="~~Ages 5 to 10")
printPAR(AR35cfu.diar.11plus,colSums(N.age11plus),label="~~Ages >10")
@
& \\
\textbf{Gastrointestinal} \\
\textbf{illness$^4$, episodes} \\
<<'AR watexp gi overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600-gi.Rdata")
printPAR(AR35cfu.gi,colSums(N.all),label="All Ages")
@
Age Stratified \\
<<'AR watexp gi overall ages',echo=FALSE,results='asis'>>=
printPAR(AR35cfu.gi.0to4,colSums(N.age0to4),label="~~Ages 0 to 4")
printPAR(AR35cfu.gi.5to10,colSums(N.age5to10),label="~~Ages 5 to 10")
printPAR(AR35cfu.gi.11plus,colSums(N.age11plus),label="~~Ages >10")
@
& \\
\textbf{Missed Daily} \\
\textbf{Activities$^5$, days} \\
<<'AR watexp dailygi overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600-dailygi.Rdata")
printPAR(AR35cfu.dailygi,colSums(N.all),label="All Ages")
@
Age Stratified \\
<<'AR watexp dailygi overall ages',echo=FALSE,results='asis'>>=
# TEMP CODE
# zero-out a few unrealistic estimates that weren't filtered in ARboot
#AR35cfu.dailygi.0to4$stats[(AR35cfu.dailygi.0to4$stats[,3]>10), ] <- NA
  AR35cfu.dailygi.0to4$bootest  <- apply(AR35cfu.dailygi.0to4$stats,2,function(x) mean(x,na.rm=T))
  AR35cfu.dailygi.0to4$boot95lb <- apply(AR35cfu.dailygi.0to4$stats,2,function(x) quantile(x,prob=c(0.025),na.rm=T))
  AR35cfu.dailygi.0to4$boot95ub <- apply(AR35cfu.dailygi.0to4$stats,2,function(x) quantile(x,prob=c(0.975),na.rm=T))
printPAR(AR35cfu.dailygi.0to4,colSums(N.age0to4),label="~~Ages 0 to 4")
printPAR(AR35cfu.dailygi.5to10,colSums(N.age5to10),label="~~Ages 5 to 10")
printPAR(AR35cfu.dailygi.11plus,colSums(N.age11plus),label="~~Ages >10")
@
& \\
\textbf{Missed Paid} \\
\textbf{Work$^6$, days} \\
<<'AR watexp workgi overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600-workgi.Rdata")
printPAR(AR35cfu.workgi,colSums(N.all),label="All Ages\\textsuperscript{8}")
@
& \\
\textbf{Medical Visits$^7$,} \\
\textbf{events} \\
<<'AR watexp medgi overall',echo=FALSE,results='asis'>>=
load("~/dropbox/13beaches/aim2-results/rawoutput/aim2-PARentero1600-medgi.Rdata")
printPAR(AR35cfu.medgi,colSums(N.all),label="All Ages\\textsuperscript{8}")
@
& \\
\hline
\end{tabular}
...
\begin{enumerate}
  ...
  \item Population Attributable Risk is the number of events per 1000 swimmers that would be prevented if the exposure of swimming in water with \textit{Enterococcus} $\geq$35 CFU/100ml were removed from the population. The proportion of swimmers exposed to water with \textit{Enterococcus} >35 CFU/100ml was: all ages (\Sexpr{pctexp[1]}), ages 0-4 (\Sexpr{pctexp[2]}), ages 5-10 (\Sexpr{pctexp[3]}), ages >10 (\Sexpr{pctexp[4]}). 
...
%   \setlength{\itemsep}{1pt}
%   \item Predicted incidence per 1000 among body immersion swimmers under the empirical distribution of \textit{Enterococcus} exposure (observed) and under a counterfactual scenario where water conditions never exceeded 35 CFU/100ml. Estimates are from a multivariable regression model adjusted for a range of potential confounders and beach level fixed-effects (see statistical analysis plan for details).
%   \item Population Attributable Risk is the number of events per 1000 swimmers that would be prevented if the exposure of swimming in water with \textit{Enterococcus} $\geq$35 CFU/100ml were removed from the population. The proportion of swimmers exposed to water with \textit{Enterococcus} >35 CFU/100ml was: all conditions (\Sexpr{pctexp[1]}), point source conditions (\Sexpr{pctexp[2]}), non-point source conditions (\Sexpr{pctexp[3]}). 
%   \item Population Attributable Fraction is the percentage of events among swimmers attributable to swimming in water with \textit{Enterococcus} $\geq$35 CFU/100ml.
%   \item Gastrointestinal illness was defined as (i) diarrhea or (ii) vomiting or (iii) stomach cramps and missed daily activities or (iv) nausea and missed daily activities.
%   \item Includes days of school, work, or vacation missed because of gastrointestinal illness.
% \end{enumerate}
