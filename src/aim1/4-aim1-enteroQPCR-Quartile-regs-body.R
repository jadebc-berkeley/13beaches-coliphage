...
# --------------------------------------
# 4-aim1-enteroQPCR-Quartile-regs-body.R
...
#
# Analyses are conducted for EPA qQPCR
# Among Swimmers with Body Immmersion
#
#   NOTE: this script is identical to 
#	2-aim1-entero1600-Quartile-regs-body.R
# 	but uses a different exposure variable
#   with a global replace of entero1600 -> enteroQPCR
#
# --------------------------------------
...
N.nps <- calcNs(ad$diarrheaci10[ad$pointsource=="No"], ad$qenteroQPCR[ad$pointsource=="No"])
N.ps  <- calcNs(ad$diarrheaci10[ad$pointsource=="Yes"],ad$qenteroQPCR[ad$pointsource=="Yes"])
# age and point source stratified
N.nps0to4   <- calcNs(ad$diarrheaci10[ad$agestrat=="(0, 4]" & ad$pointsource=="No"],ad$qenteroQPCR[ad$agestrat=="(0, 4]" & ad$pointsource=="No"] )
N.nps5to10  <- calcNs(ad$diarrheaci10[ad$agestrat=="(4, 10]" & ad$pointsource=="No"],ad$qenteroQPCR[ad$agestrat=="(4, 10]" & ad$pointsource=="No"] )
N.nps11plus <- calcNs(ad$diarrheaci10[ad$agestrat==">10" & ad$pointsource=="No"],ad$qenteroQPCR[ad$agestrat==">10" & ad$pointsource=="No"] )

N.ps0to4   <- calcNs(ad$diarrheaci10[ad$agestrat=="(0, 4]" & ad$pointsource=="Yes"],ad$qenteroQPCR[ad$agestrat=="(0, 4]" & ad$pointsource=="Yes"] )
N.ps5to10  <- calcNs(ad$diarrheaci10[ad$agestrat=="(4, 10]" & ad$pointsource=="Yes"],ad$qenteroQPCR[ad$agestrat=="(4, 10]" & ad$pointsource=="Yes"] )
N.ps11plus <- calcNs(ad$diarrheaci10[ad$agestrat==">10" & ad$pointsource=="Yes"],ad$qenteroQPCR[ad$agestrat==">10" & ad$pointsource=="Yes"] )
...
# Point source
psci <- mpreg(diarrheaci10~qenteroQPCR,dat=ad[ad$pointsource=="Yes",],vcv=T)

# Non-point source
npsci <- mpreg(diarrheaci10~qenteroQPCR,dat=ad[ad$pointsource=="No",],vcv=T)
...
av.h.noint <-glm(diarrheaci10~ qenteroQPCR+groundwater +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,family=poisson(link="log"),data=ad[ad$beach=="Avalon",])
av.h <-glm(diarrheaci10~ qenteroQPCR*groundwater +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,family=poisson(link="log"),data=ad[ad$beach=="Avalon",])
	lrtest(av.h.noint,av.h)

# Doheny
# Sand berm open vs. closed
# See Colford et al. 2012 Wat Res for details
dh.h.noint <-glm(diarrheaci10~qenteroQPCR+berm +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,family=poisson(link="log"),data=ad[ad$beach=="Doheny",])
dh.h <-glm(diarrheaci10~ qenteroQPCR*berm +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,family=poisson(link="log"),data=ad[ad$beach=="Doheny",])
...
# Huntington
hufit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Huntington",],vcv=T)

# Silver
sifit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Silver",],vcv=T)

# Washington Park
wpfit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Washington Park",],vcv=T)

# West
wefit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="West",],vcv=T)

# --------------------------------------
# Marine beaches

# Avalon
avfit <- mpreg(diarrheaci10~qenteroQPCR*groundwater +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Avalon",],vcv=T)

# Boqueron
bofit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Boqueron",],vcv=T)

# Doheny
dhfit <- mpreg(diarrheaci10~qenteroQPCR*berm +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Doheny",],vcv=T)

# Edgewater
# all of the water samples in Edgewater are in the 4th quartile of Enterococcus 1611 -- commented out
# edfit <- mpreg(diarrheaci10~ qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood ,dat=ad[ad$beach=="Edgewater",],vcv=T)

# Fairhope
fafit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Fairhope",],vcv=T)

# Goddard
gdfit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Goddard",],vcv=T)

# Malibu
mafit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Malibu",],vcv=T)

# Mission Bay
mbfit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Mission Bay",],vcv=T)

# Surfside
sufit <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood,dat=ad[ad$beach=="Surfside",],vcv=T)
...
nps <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood+beach,dat=ad[ad$pointsource=="No",],vcv=T)
ps <- mpreg(diarrheaci10~qenteroQPCR +agecat+female+racewhite+gichron+anim_any+gicontactbase+rawfood+beach,dat=ad[ad$pointsource=="Yes",],vcv=T)
...
nps0to4 <- mpreg(diarrheaci10~qenteroQPCR +female+racewhite+gichron+anim_any+gicontactbase+rawfood+beach,dat=ad[ad$pointsource=="No" & ad$agestrat=="(0, 4]",],vcv=T)

nps5to10 <- mpreg(diarrheaci10~qenteroQPCR +female+racewhite+gichron+anim_any+gicontactbase+rawfood+beach,dat=ad[ad$pointsource=="No" & ad$agestrat=="(4, 10]",],vcv=T)

nps11plus <- mpreg(diarrheaci10~qenteroQPCR +female+racewhite+gichron+anim_any+gicontactbase+rawfood+beach,dat=ad[ad$pointsource=="No" & ad$agestrat==">10",],vcv=T)

# Point-source estimates by age group
ps0to4 <- mpreg(diarrheaci10~qenteroQPCR +female+racewhite+gichron+anim_any+gicontactbase+rawfood+beach,dat=ad[ad$pointsource=="Yes" & ad$agestrat=="(0, 4]",],vcv=T)

ps5to10 <- mpreg(diarrheaci10~qenteroQPCR +female+racewhite+gichron+anim_any+gicontactbase+rawfood+beach,dat=ad[ad$pointsource=="Yes" & ad$agestrat=="(4, 10]",],vcv=T)

ps11plus <- mpreg(diarrheaci10~qenteroQPCR +female+racewhite+gichron+anim_any+gicontactbase+rawfood+beach,dat=ad[ad$pointsource=="Yes" & ad$agestrat==">10",],vcv=T)
...
CIRquartiles <- function(x,vcv) {
	...
	rownames(cirs) <- paste("EnteroQPCR-Q",2:4,sep="")
	return(cirs)
}
...
save(
	...
	hufit,sifit,wpfit,wefit,avfit,bofit,dhfit,fafit,gdfit,mafit,mbfit,sufit,
	# edfit,
	...
	file="~/dropbox/13beaches/aim1-results/rawoutput/aim1-enteroQPCR-Quartile-regs-body.Rdata"
	)
