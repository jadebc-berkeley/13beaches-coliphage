...
# Avalon
avfit <- lbreg(gici10~avgdyenteropcr,dat=d,beach="Avalon")

# Boqueron
bofit <- lbreg(gici10~avgdyenteropcr,dat=d,beach="Boqueron")

# Doheny
dhfit <- lbreg(gici10~avgdyenteropcr,dat=d,beach="Doheny")
...
# Avalon
avcfit <- lbreg(gici10~avgdyenteropcr,dat=dch,beach="Avalon")

# Boqueron
bocfit <- lbreg(gici10~avgdyenteropcr,dat=dch,beach="Boqueron")

# Doheny
dhcfit <- lbreg(gici10~avgdyenteropcr,dat=dch,beach="Doheny")
...
rownames(mallests) <- beachnmsm
rownames(mchests) <- beachnmsm
...
# total population
allpdata <- data.frame(rbind(fallests,mallests))
colnames(allpdata) <- c("est","lb","ub")
allpdata$type<-factor(c(rep("Fresh",4),rep("Marine",8)))
...
# children <=10
chpdata <- data.frame(rbind(fchests,mchests))
colnames(chpdata) <- c("est","lb","ub")
chpdata$type<-factor(c(rep("Fresh",4),rep("Marine",8)))
...
pdf("~/dropbox/13beaches/aim1-results/figs/cirplot-epaQPCR-all2.pdf",width=6,height=8)
...
ylims <- c(0.1,8)
pdf("~/dropbox/13beaches/aim1-results/figs/cirplot-epaQPCR-child2.pdf",width=6,height=8)
