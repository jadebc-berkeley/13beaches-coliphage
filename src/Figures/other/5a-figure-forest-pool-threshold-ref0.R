...
load("~/Documents/CRG/coliphage/results/rawoutput/regress-10day-body-pool-threshold-ref0.Rdata")
...
gici10.plot=rbind(gici10.plot.pool,gici10.plot.high,gici10.plot.low,
                  gici10.plot.pool.25,gici10.plot.high.25,gici10.plot.low.25,
...
gici10.plot$lab=as.factor(rep(c(rep("All conditions",2),rep("Human fecal pollution\n likely present",2),
                                rep("Human fecal pollution\n likely not present",2)),4))

gici10.plot$ind=as.factor(rep(c("Somatic Coliphage","Male-Specific Coliphage"),12))

gici10.plot$type=as.factor(c(rep("Any detected coliphage",6),
                             rep(">25% of samples detected coliphage",6),
...
gici10.plot$type.f=factor(gici10.plot$type,
                          levels=c(
                                    ">75% of samples detected coliphage",
                                   ">50% of samples detected coliphage",
                                   ">25% of samples detected coliphage",
                                   "Any detected coliphage"))


pdf("~/Documents/CRG/coliphage/results/figures/forestplots_pool_threshold_ref0.pdf",height=5,width=9)
ggplot(gici10.plot,aes(x=lab.f,y=pt.est))+
  ...
  geom_point(aes(color=type.f,shape=type.f,fill=type.f),position=position_dodge(width=0.4))+
  coord_flip()+ geom_hline(yintercept=1,linetype="dotted")+
  scale_color_manual("",breaks=rev(levels(gici10.plot$type.f)),values=c("#a366ff","#6600ff","#1f004d","#000000"))+
  scale_shape_manual("",breaks=rev(levels(gici10.plot$type.f)),values=c(23,15,17,19))+  
  scale_fill_manual("",breaks=rev(levels(gici10.plot$type.f)),values=c("#a366ff","#6600ff","#1f004d","#000000"))+  
